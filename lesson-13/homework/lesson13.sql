...
--create table Employee (EMPLOYEE_ID int, FIRST_NAME varchar(50), LAST_NAME varchar(50),EMAIL varchar(255), PHONE_NUMBER varchar(50),HIRE_DATE date,JOB_ID varchar(50),SALARY float,COMMISSION_PCT float, MANAGER_ID int, DEPARTMENT_ID int)
...
--INSERT INTO Employeed (EmployeeID, ManagerID, JobTitle) VALUES
...
--INSERT INTO [Order] (CustomerID, OrderID, DeliveryState, Amount) VALUES
...
--1
select cast(employee_id as char) + '-'+ ' ' + first_name +' ' + last_name from employee
--2
UPDATE employee
...
--4
select manager_id, SUM(salary) total_salary from employee
Group by manager_id
--5
select  year1, case
when
max1 > max2  and max1> max3 then max1
when
max2 > max1 and max2> max3 then max2
else max3
end
from testmax
--6
...
SUBSTRING(fullname, CHARINDEX(' ', fullname) -1, CHARINDEX( )
from students
-- 2
select * from [order]
where customerid in 
(select DISTINCT customerid from [order] where deliverystate = 'CA')
...
FROM 
    Employee
WHERE 
    LEN(REPLACE(CONCAT(First_Name, Last_Name), 'a', '')) <= LEN(CONCAT(First_Name, Last_Name)) - 3;
--5
select(select COUNT(*) from employee
)/(select COUNT(*) from employee
where YEAR(GETDATE()) - YEAR(Hire_Date) >2
GROUP BY YEAR(HIRE_DATE)) * 100
-- 6
SELECT P.* FROM PERSONAL P JOIN (SELECT MIN(MISSIONCOUNT) MISSIONCOUNT FROM PERSONAL
UNION ALL
SELECT MAX(MISSIONCOUNT) FROM PERSONAL) MINMAX
ON P. MISSIONCOUNT = MINMAX.MISSIONCOUNT
...
SELECT 
    FULLNAME, 
    GRADE,
    SUM(GRADE) OVER (ORDER BY studentid) AS cumulative_score
FROM Students;
SELECT 
    a.FULLNAME,
    a.GRADE,
    (SELECT SUM(b.GRADE) 
     FROM Students b 
     WHERE b.studentid <= a.studentid) AS cumulative_score
FROM Students a;
--3
...
join student t
on s.birthday = t.birthday and s.studentname <> t.studentname
--5
